...
def structural_policy():
    return make_structural_validation_policy(
        policy_ref="policy:structure:rep-runtime",
...
def build_confirmed_chain():
    ...
    search_policy = make_search_budget_policy(
        policy_ref="policy:search:rep-runtime", subject_identity_ref=IDENTITY,
        max_unique_evaluations=4, base_min_regularized_improvement_ppm=0, multiplicity_penalty_ppm=0, registered_at=20,
    )
    ...
    search_review = review_search_budget_selection(
    ...
    cp = make_confirmation_policy(
        policy_ref="policy:confirm:rep-runtime", subject_identity_ref=IDENTITY,
    ...
    batch = make_confirmation_batch(
        batch_ref="batch:confirm:rep-runtime", subject_identity_ref=IDENTITY, pair_key=PAIR_KEY,
        source_ref="source:confirm-a", samples=confirmation_rows, batch_keeper_ref="confirm-batch", sealed_at=650,
    )
    assert_confirmation_freshness(batch=batch, search_samples=history, prior_batches=())
    exposure = authorize_confirmation_exposure(
        selection=selection, search_review=search_review, candidate=candidate, batch=batch, policy=cp,
        prior_exposures=(), exposure_keeper_ref="confirm-exposure", authorized_at=670,
    )
    _, confirmation_eval = evaluate_confirmation(
        selection=selection, candidate=candidate, batch=batch, exposure=exposure,
        confirmation_samples=confirmation_rows, search_samples=history, current_graph=base_graph,
        structural_policy=sp, confirmation_policy=cp, evaluator_ref="confirm-evaluator", evaluated_at=700,
    )
    confirmation_review = review_confirmation(
        evaluation=confirmation_eval, decision="APPROVE", rationale_ref="confirm-review",
    ...
    confirmed_graph, revision = apply_confirmed_selection(
        ...
        search_evaluation=search_eval, search_selection=selection, search_review=search_review,
        exposure=exposure, confirmation_evaluation=confirmation_eval, confirmation_review=confirmation_review,
        applier_ref="confirm-applier", applied_at=720,
    )
    return (
        sp, base_graph, confirmed_graph, history, candidate, reservation, underlying, search_eval,
        selection, search_review, batch, confirmation_rows, exposure, confirmation_eval, confirmation_review, revision,
    )
...
def seed_db(db_path):
    ...
    chain = build_confirmed_chain()
    sp, base_graph, confirmed_graph, history, candidate, reservation, underlying, search_eval, selection, search_review, batch, confirmation_rows, exposure, confirmation_eval, confirmation_review, revision = chain
    ...
    conn.execute("""INSERT INTO protected_confirmation_revision_history(
        ...
        json.dumps(graph_to_dict(base_graph), sort_keys=True, separators=(",", ":")),
        json.dumps(graph_to_dict(confirmed_graph), sort_keys=True, separators=(",", ":")),
        json.dumps(confirmation_batch_to_dict(batch), sort_keys=True, separators=(",", ":")),
        json.dumps(exposure_to_dict(exposure), sort_keys=True, separators=(",", ":")),
        json.dumps(confirmation_evaluation_to_dict(confirmation_eval), sort_keys=True, separators=(",", ":")),
        json.dumps(confirmation_review_to_dict(confirmation_review), sort_keys=True, separators=(",", ":")),
        json.dumps(search_selection_to_dict(selection), sort_keys=True, separators=(",", ":")),
        json.dumps(search_review_to_dict(search_review), sort_keys=True, separators=(",", ":")),
        json.dumps(candidate_to_dict(candidate), sort_keys=True, separators=(",", ":")),
        json.dumps(reservation_to_dict(reservation), sort_keys=True, separators=(",", ":")),
        json.dumps(validation_to_dict(underlying), sort_keys=True, separators=(",", ":")),
        json.dumps(search_evaluation_to_dict(search_eval), sort_keys=True, separators=(",", ":")),
...
def grant_for(root, actor, *, subject, role):
    return issue_authority_grant(
        ...
        valid_from=100, valid_until=5000, issuer_ref="root", issuer_key_id="root-key", issuer_private_key=root,
    )
...
def setup_target(db_path):
    chain = seed_db(db_path)
    sp, base_graph, confirmed_graph, history, candidate, _, _, _, _, _, confirmation_batch, _, _, confirmation_eval, confirmation_review, revision = chain
    root = Ed25519PrivateKey.generate()
    policy_key = Ed25519PrivateKey.generate(); policy_grant = grant_for(root,policy_key,subject="rep-policy",role=ROLE_REPLICATION_POLICY_KEEPER)
    policy = make_replication_policy(
        policy_ref="policy:replication:runtime", subject_identity_ref=IDENTITY,
        min_replication_samples=4, min_temporal_gap=100, min_regularized_improvement_ppm=0,
        max_proposed_brier_degradation_ppm=100_000, persistent_drift_epochs=2, registered_at=800,
    )
    req = signed_request("register_replication_policy", {
        "policy_ref":policy.policy_ref,"subject_identity_ref":IDENTITY,"min_replication_samples":4,"min_temporal_gap":100,
        "min_regularized_improvement_ppm":0,"max_proposed_brier_degradation_ppm":100_000,"persistent_drift_epochs":2,
    }, action_register_policy(policy), key=policy_key, grant=policy_grant, role=ROLE_REPLICATION_POLICY_KEEPER, request_id="policy", now=800)
    completed,response=invoke(req,root=root,db_path=db_path,now=800); assert completed.returncode==0,response

    target_key = Ed25519PrivateKey.generate(); target_grant = grant_for(root,target_key,subject="rep-target",role=ROLE_REPLICATION_TARGET_KEEPER)
    target = make_replication_target(
        target_ref="target:runtime", confirmed_revision=revision, confirmation_evaluation=confirmation_eval,
        confirmation_review=confirmation_review, confirmation_batch=confirmation_batch, candidate=candidate,
        confirmed_graph=confirmed_graph, registered_at=810,
    )
    req = signed_request("register_replication_target", {"target_ref":target.target_ref,"confirmed_revision_hash":revision.revision_hash}, action_register_target(target), key=target_key, grant=target_grant, role=ROLE_REPLICATION_TARGET_KEEPER, request_id="target", now=810)
    completed,response=invoke(req,root=root,db_path=db_path,now=810); assert completed.returncode==0,response
    return root,policy,target,sp,base_graph,confirmed_graph,history,candidate


def setup_evaluation(db_path, *, correlated=True):
    root,policy,target,sp,base_graph,confirmed_graph,history,candidate = setup_target(db_path)
    rows = replication_rows(correlated=correlated)
    batch_key = Ed25519PrivateKey.generate(); batch_grant = grant_for(root,batch_key,subject="rep-batch",role=ROLE_REPLICATION_BATCH_KEEPER)
    batch = make_replication_batch(
        batch_ref="batch:runtime:0", target=target, policy=policy, mode="TEMPORAL_EXTERNAL",
        source_ref="source:lab-b", environment_ref="env:lab-b", samples=rows,
        collected_from=1000, collected_to=1200, generation=0, previous_batch=None,
        batch_keeper_ref="rep-batch", sealed_at=1300,
    )
    req = signed_request("seal_replication_batch", {
        "target_hash":target.target_hash,"batch_ref":batch.batch_ref,"mode":batch.mode,"source_ref":batch.source_ref,
        "environment_ref":batch.environment_ref,"collected_from":1000,"collected_to":1200,"samples":[pair_to_dict(item) for item in rows],
    }, action_seal_batch(batch), key=batch_key, grant=batch_grant, role=ROLE_REPLICATION_BATCH_KEEPER, request_id="batch", now=1300)
    completed,response=invoke(req,root=root,db_path=db_path,now=1300); assert completed.returncode==0,response

    evaluator_key = Ed25519PrivateKey.generate(); evaluator_grant = grant_for(root,evaluator_key,subject="rep-evaluator",role=ROLE_REPLICATION_EVALUATOR)
    _, evaluation = evaluate_replication(
        ...
        base_graph=base_graph,confirmed_graph=confirmed_graph,structural_policy=sp,replication_policy=policy,
        evaluator_ref="rep-evaluator",evaluated_at=1400,
    )
    req = signed_request("evaluate_replication", {"batch_hash":batch.batch_hash}, action_evaluate(evaluation), key=evaluator_key, grant=evaluator_grant, role=ROLE_REPLICATION_EVALUATOR, request_id="evaluation", now=1400)
    completed,response=invoke(req,root=root,db_path=db_path,now=1400); assert completed.returncode==0,response
...
def test_runtime_replication_survives_processes_review_and_snapshot(tmp_path):
    ...
    root,policy,target,_,evaluation = setup_evaluation(db_path,correlated=True)
    assert evaluation.status == "REPLICATED"
    reviewer_key=Ed25519PrivateKey.generate(); reviewer_grant=grant_for(root,reviewer_key,subject="rep-reviewer",role=ROLE_REPLICATION_REVIEWER)
    review=review_replication(evaluation=evaluation,decision="ACKNOWLEDGE",rationale_ref="replication-reviewed",reviewer_ref="rep-reviewer",reviewed_at=1500)
    req=signed_request("record_replication_review",{"evaluation_hash":evaluation.evaluation_hash,"decision":"ACKNOWLEDGE","rationale_ref":"replication-reviewed"},action_review(review),key=reviewer_key,grant=reviewer_grant,role=ROLE_REPLICATION_REVIEWER,request_id="review",now=1500)
    completed,response=invoke(req,root=root,db_path=db_path,now=1500); assert completed.returncode==0,response

    snapshot=summarize_replication_series(evaluations=(evaluation,),reviews=(review,),policy=policy,measured_at=1600)
    monitor_key=Ed25519PrivateKey.generate(); monitor_grant=grant_for(root,monitor_key,subject="rep-monitor",role=ROLE_REPLICATION_MONITOR_KEEPER)
    req=signed_request("finalize_replication_snapshot",{"target_hash":target.target_hash},action_snapshot(snapshot),key=monitor_key,grant=monitor_grant,role=ROLE_REPLICATION_MONITOR_KEEPER,request_id="snapshot",now=1600)
    completed,response=invoke(req,root=root,db_path=db_path,now=1600); assert completed.returncode==0,response
...
def test_runtime_external_replication_rejects_original_confirmation_source(tmp_path):
    db_path = tmp_path / "runtime.sqlite3"
    root,policy,target,_,_,_,_,_ = setup_target(db_path)
    rows = replication_rows()
    batch_key=Ed25519PrivateKey.generate(); batch_grant=grant_for(root,batch_key,subject="rep-batch",role=ROLE_REPLICATION_BATCH_KEEPER)
    temporal = make_replication_batch(
        batch_ref="signed-shape",target=target,policy=policy,mode="TEMPORAL",source_ref=target.confirmation_source_ref,
        environment_ref="env",samples=rows,collected_from=1000,collected_to=1200,generation=0,previous_batch=None,
        batch_keeper_ref="rep-batch",sealed_at=1300,
    )
    req=signed_request("seal_replication_batch",{
        "target_hash":target.target_hash,"batch_ref":"bad-external","mode":"EXTERNAL","source_ref":target.confirmation_source_ref,
        "environment_ref":"env","collected_from":1000,"collected_to":1200,"samples":[pair_to_dict(item) for item in rows],
    },action_seal_batch(temporal),key=batch_key,grant=batch_grant,role=ROLE_REPLICATION_BATCH_KEEPER,request_id="bad-source",now=1300)
    completed,response=invoke(req,root=root,db_path=db_path,now=1300)
...
def test_runtime_stale_target_rejects_evaluation_after_graph_change(tmp_path):
    ...
    batch_key=Ed25519PrivateKey.generate(); batch_grant=grant_for(root,batch_key,subject="rep-batch",role=ROLE_REPLICATION_BATCH_KEEPER)
    batch=make_replication_batch(
        batch_ref="batch:stale",target=target,policy=policy,mode="TEMPORAL_EXTERNAL",source_ref="source:lab-b",environment_ref="env",
        samples=rows,collected_from=1000,collected_to=1200,generation=0,previous_batch=None,batch_keeper_ref="rep-batch",sealed_at=1300,
    )
    req=signed_request("seal_replication_batch",{"target_hash":target.target_hash,"batch_ref":batch.batch_ref,"mode":batch.mode,"source_ref":batch.source_ref,"environment_ref":batch.environment_ref,"collected_from":1000,"collected_to":1200,"samples":[pair_to_dict(item) for item in rows]},action_seal_batch(batch),key=batch_key,grant=batch_grant,role=ROLE_REPLICATION_BATCH_KEEPER,request_id="batch",now=1300)
    completed,response=invoke(req,root=root,db_path=db_path,now=1300); assert completed.returncode==0,response
    ...
    evaluator_key=Ed25519PrivateKey.generate(); evaluator_grant=grant_for(root,evaluator_key,subject="rep-evaluator",role=ROLE_REPLICATION_EVALUATOR)
    _,evaluation=evaluate_replication(target=target,batch=batch,replication_samples=rows,search_samples=history,candidate=candidate,base_graph=base_graph,confirmed_graph=confirmed_graph,structural_policy=sp,replication_policy=policy,evaluator_ref="rep-evaluator",evaluated_at=1400)
    req=signed_request("evaluate_replication",{"batch_hash":batch.batch_hash},action_evaluate(evaluation),key=evaluator_key,grant=evaluator_grant,role=ROLE_REPLICATION_EVALUATOR,request_id="stale",now=1400)
    completed,response=invoke(req,root=root,db_path=db_path,now=1400)
    assert completed.returncode == 2
    assert "no longer represents current model" in response["error"]


def test_runtime_wrong_role_cannot_record_review(tmp_path):
    db_path = tmp_path / "runtime.sqlite3"
    ...
    review=review_replication(evaluation=evaluation,decision="ACKNOWLEDGE",rationale_ref="drift-seen",reviewer_ref="wrong-review",reviewed_at=1500)
    req=signed_request("record_replication_review",{"evaluation_hash":evaluation.evaluation_hash,"decision":"ACKNOWLEDGE","rationale_ref":"drift-seen"},action_review(review),key=wrong_key,grant=wrong_grant,role=ROLE_REPLICATION_EVALUATOR,request_id="wrong-role",now=1500)
    completed,response=invoke(req,root=root,db_path=db_path,now=1500)
    assert completed.returncode == 2
